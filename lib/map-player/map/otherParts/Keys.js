...
export class Keys {
    ...
    constructor(root){
        ...
        this.keyA.src = 'assets/images/keys/A_Key_Dark.png';
        ...
        this.keySpace.onload=function(){root.menu.alphaTarget=.6}
    ...
    drawKeys() {
        ...
        this.drawImageToGrid(this.keyA,70,43,2,2)
        this.functionToGrid(this.jump,82,125)
        // this.functionToGrid(this.jump,92,33)
        // this.functionToGrid(this.jump,84,25)
        // this.functionToGrid(this.State,444,22,0,"bazooka",3,3)
        // this.functionToGrid(this.State,7,12,0,"hook",1,1)
        // this.functionToGrid(this.State,6,12,0,"pickaxe",1,1)
        // this.functionToGrid(this.State,5,12,0,"bazooka",1,1)
        // this.functionToGrid(this.State,4,12,0,"shotgun",1,1)
        // this.functionToGrid(this.State,3,12,0,"dash",1,1)
        // this.functionToGrid(this.State,2,12,0,"portal",1,1)//7,12 - 2,12
        // this.drawImageToGrid(this.leftClick,466,24,2,2)
    ...
    functionToGrid(func, gridX, gridY, type, item, gridWidth, gridHeight){
        ...
        if(func==this.Tip){
            this.Tip(x,y,type,item)
        }
        if(func==this.State){
            this.State(x,y,item, width, height)
        }
    }
    ...
    State(x, y, item = "hook", width=100, height=100) {


        if(this.game.player.Held.get(item)) {
            ctx.drawImage(this.Enabled.get(item), x, y, width, height)
        } else {
            ctx.drawImage(this.Disabled.get(item), x, y, width, height)
        }
    } //466, 24
