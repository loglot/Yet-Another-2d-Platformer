...
export class Edit {
    ...
    tick(){
      ...
      if(kd.PLUS.isDown()){
        this.selected=Math.round((10+this.selected)*100)/100
        keythisframe=true
      }
      if(kd.BRACKETR.isDown()){
        this.selected=Math.round((100+this.selected)*100)/100
        keythisframe=true
      }
      if(kd.BRACKETL.isDown()){
        this.selected=Math.round((-100+this.selected)*100)/100
        keythisframe=true
      }
      if(kd.MINUS.isDown()){
        this.selected=Math.round((.01+this.selected)*100)/100
        keythisframe=true
      }
      ...
      if(this.click){
        if(!this.game.map.mapBuffer[this.gridX]){
          ...
          this.game.map.clearPixel(this.gridX+100,this.gridY)
          this.game.map.changePixel(this.gridX+100,this.gridY,this.selected)
        }
      }
      if(!this.saved){
