...
export class Player{
    ...
    move(){
        ...
        var xRange=[
            Math.max(Math.round(this.x/100)-3, 0), 
            Math.min(this.game.map.ground.mapBuffer.length, 6)]
        
        for(let x = xRange[1];x < xRange[2]; x++){
            var yRange=[
                Math.max(Math.round(this.y/100)-3, 0), 
                Math.min(this.game.map.ground.mapBuffer[x].length, 6)]

            for(let y = xRange[1];y < yRange[2]; y++){
