export class Bazooka{
    ...
    destroy(){
        const xRange=[Math.max(Math.round(this.x/50-this.explodeR/50),0),Math.min(Math.round(this.x/50+this.explodeR/50),this.game.map.mapBuffer.length)]
        const yRange=[Math.round(this.y/50-this.explodeR/50),Math.round(this.y/50+this.explodeR/50)]
        this.velX = 0
        this.velY = 0
