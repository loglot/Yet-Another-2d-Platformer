...
export class Display{
    ...
    tick(){
            ...
            this.game.map.bgDraw(this.game.camera)
            this.drawHeld()
            this.game.player.drawPlayer()
            this.game.map.nnDraw(this.game.camera)
            this.game.player.drawDeath()
            ...
            if(kd.M.isDown() && !kd.BACKSLASH.isDown()){
                if(!this.MPressedLastFrame){

                    this.minimapDisplay = !this.minimapDisplay
                }
                this.MPressedLastFrame=true
            } else{
                
                this.MPressedLastFrame=false
            }
        
            if(this.minimapDisplay){
            
            
                this.miniMapWidth = ((this.miniMapWidth*5) + 1355)/6
            } else {
                this.miniMapWidth = ((this.miniMapWidth*5))/6
            }
            if(this.miniMapWidth>10){
                ctx.beginPath();
                ctx.rotate(.7)
                ctx.rect(-200,-2000,this.miniMapWidth+10,4000)
                ctx.fillStyle= "#303040"
                ctx.fill()
                ctx.beginPath();


                ctx.rect(-200,-2000,this.miniMapWidth,4000)
                ctx.clip()
                ctx.rotate(-.7)
                this.drawMiniMap()
    ...
    drawHeld(){
        ...
        if(this.game.held.pickaxe.enabled){

            this.game.draw.Line(
                this.game.held.pickaxe.x[0],
                this.game.held.pickaxe.y[0],
                this.game.held.pickaxe.x[1],
                this.game.held.pickaxe.y[1],
                "#6b4101", 
                30/* + -this.game.held.hook.length/70*/, 
                true, "#A06000" 
            )
